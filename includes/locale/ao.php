<?php
$lang = array(
    "LANGUAGE" => ' Afaan ',
    "HULUTERA_TEXT_LOGO" => '<span style="color:orange">Hulluu</span><span style="color:#050598a6">taraa</span>',
    "HULUTERA_TEXT_LOGO2" => '<span style="color:orange">Hulluutaraa</span>',
    "Post Items" => 'Qabeenya galchaa ',
    "Help" => ' Odeefannoo ',
    "Show Detail" => 'Tarreefamicha ilaaluf ',
    "Hide Detail" => ' Tarreefamicha aguguuf ',
    "Contact" => 'Qunamu ',
    "Contact method" => ' Haala itti qunnaman ',
    "change contact method" => 'Haala ittiin qunnaman jijjiiruf ',
    "change your email" => ' Imeelii jijjiiruuf ',
    "e-mail" => ' Imeelii ',
    "new e-mail" => ' Imeelii haaraa ',
    "save" => ' Haa jijjiiramu ',
    "save changes" => ' Inni jijjiirame haa taa’u ',
    "cancel changes" => ' Hin jijjiirin ',
    "edit" => ' Jijjiiruf ',
    "Edit Profile" => ' Odeefannoo sirreessuuf ',
    "Please enter a username" => ' Maqaa ittiin fayyadamtan galchaa ',
    "Username must consist of letters and numbers only" => ' Maqaan ittiin fayyadamtan qubee fi lakkoofsa qofaa qabaachuu qaba. ',
    "Please enter a password" => ' Lakkoofsa iccitii galchaa ',
    "Passwords do not match" => ' Jechoonni icciitiichaa walfakkaachuu qabu ',
    "Firstname must consist of letters and numbers only" => ' Maqaa keessan galchaa ',
    "Enter your Firstname" => ' Maqaa keessan galchaa ',
    "Lastname must consist of letters and numbers only" => ' Maqaa abbaa keessanii qofaa galchaa ',
    "Enter your Lastname" => ' Maqaa abbaa keessanii galchaa ',
    "Phone must be a number" => ' Lakkofsa qofaa galchaa ',
    "username" => ' Maqaa ittiin fayyadamtan galchaa ',
    "First Name" => ' Maqaa ',
    "Full Name" => ' Maqaa guutuu ',
    "Last Name" => ' Maqaa abbaa ',
    "Email" => ' Imeelii ',
    "Phone" => ' Bilbila ',
    "password" => ' Password ',
    "Password Again" => ' Password irraa deebiin ',
    "Please Agree to the Terms and Conditions" => ' Dambii ittiin bulmaata irratti waliigaluu keessaniif kan agarsiisu filadha ',
    "I have read and agree to the terms and conditions" => ' Akkaataa qajeelfamichi hojii irra itti oolu dubbise irratti waliigalen jira. ',
    "click to see" => ' Ilaaluf kana tuqa. ',
    "Register" => ' Galmaa’a ',
    "Join to hulutera" => ' Hulluutaraatti makamaa ',
    "submit" => ' Ergaa, galchaa, gadiqabaa ',
    "privacy policy paragraph1 text" => ' haOdeefannoo dhuunfaa keessanii hin gurgurru, hin tamsaasnu, akkasumas hayyama keessan mal.ee qaama biraf hin kenninu. Garuu seeran yoo gaafatamne agarsioisuf dirqama qabna. ',
    "This privacy policy sets out how hulutera uses and protects any information that you give hulutera when you use this website.hulutera are committed to ensuring that your privacy is protected. Should we ask you to provide certain information by which you can be identified when using this website, then you can be assured that it will only be used in accordance with this privacy statement. hulutera may change this policy from time to time by updating this page. You should check this page from time to time to ensure that you are happy with any changes." => ' Qjeel;fa dhuunfaa kana irratti inni itti fayyadamu informeshiniin gosa kamiyyuu Hulluutaraa irratti galche akkamitti akka ittin fayyadamuu fi akkamitti akka eegnu ni ibsina. Hulluutaraan mirgaa dhuunfaa itti fahyyadamaa eeguf cimse ni hojjata. Yammuu website keenyatti fayyadamtan akka eenyuu taatan adda baasuuf waa’ee eenyuummaa keessanii odeefannoo galchuu kan qabdan hoggaa tahu, odeefannoon keesan kunis bu’uura qajeelfama dhuunfaa tiinkan eegamu ta’u. Hulluutaraan qajeelfama kana yeroo irraa gara yerootti itti fayyadamtoota isaaf beeksiisun jijjjiiru ni danda’a. ',
    "What we collect" => ' Akkaa galchitan kan isin gaafannuu ',
    "Your name" => ' Maqaa ',
    "Contact Information including email address" => ' Odeefannoo itti argamtan jechuunis bilbila, imeelii kkf... ',
    "What we do with the information we gather" => ' Odeefannoowwan galvhitan akkamiti akka itti fayyadamnu ',
    "We may use the information to improve our services." => ' Tajaajila keenya foyyeessuuf itti fayyadamu ni dandeenya. ',
    "We may periodically send promotional emails msg" => ' Bu’aalee haaraa fi wantootaa isin ni fayyadu jedheen yaadne adda addaa erguf itti fayyadamu ni dandeenya. ',
    "From time to time send info msg" => ' Websitichi haala yeroo wajajin kan deemuu fi fedhii keessan kan guutuu ta’usaaisin gaafachuu fi itti fayyadamu ni dandeenya. ',
    "Links to other websites" => ' Websiyitoota biroo ',
    "Our website may contain links to other websites of interest. However, once you have used these links to leave our site, you should note that we do not have any control over that other website. Therefore, we cannot be responsible for the protection and privacy of any information which you provide whilst visiting such sites and such sites are not governed by this privacy statement. You should exercise caution and look at the privacy statement applicable to the website in question." => ' Website keenya irratti websittoonni biroo mul’achuu ni danda’u. Yeroo kamiyyuu websitootaa gadqabdanii gara fuula isaaniitti yoo deeme sana booda Hulluutaraan itti gaafatamummaa hin qabu, Kanaafuu qajeelfamni dhunfaa kun fuuloota kanneen kan hammatu hin ta’u ',
    "hulutera Admin" => ' Bulchiinsa Hulluutaraa ',
    "Company" => ' Dhaabbata ',
    "Subject" => ' Mata duree ',
    "Please enter your name" => ' Maloo maqaa keesan galchaa ',
    "Please Enter only your first name" => ' Maloo maqaa keesan oqffAa galchaa ',
    "Please enter your email" => ' Maloo imeelii keessan galchaa ',
    "Your e-mail address is invalid" => ' Imeelichi sirrii miti ',
    "Please enter a subject" => ' Maalloo kaaayyo dhaamsichaa galchaa ',
    "Please state choose your purpose" => ' Maloo waa’ee maalii argachuu barbaaaddan ',
    "This is a confirmation mail from www.hulutera.com. We appreciate you for taking time to contact us.\n\n Sincerely,hulutera Admin\n\n" => ' Dhaamsi mirkaneessa kun kan isiniif ergame www.hulutera.com irraa dha, Nu argachuf yaalii gochuu keessaniif isin galatefanna. \n\n Bulchiinsa Hulluutaraa irraa ',
    "Message" => ' Dhaamsa ',
    "Send" => ' Dhaamsa ergaa ',
    "Contact Us" => ' Nu gaafachuu barbaadduu ',
    "about us page paragraph1 text" => ' Dh Hulluutaraan website bitaa fi gurgurtaa kafaltii malee kan itti fayuyadamtani hoggaa ta’u websiyiticha irratti qabeenyawwan adda addaa bitachuf, gurguruufi kireessjuu ni dandeessu. Akkasumas websiyiticha irrati qabeenyawan faayidaa irraa oolanois ta’e ka faayidaarraa hin oole ni argattu. Yero ammati Hulluutaraan Itiyoophiyaa qofaaf baayyee salphaa dha. gara fuulduratti sochii isa haala guddaan babbalisiif kaayyoo qabatee sochoo’ irratti argama. Hulluuutaraa irratti filannoowwan bal’aatu argamu. Isaani kan keessas kanneen armaan gadi warra gurguddoo dha. ',
    "hulutera is designed to fit the needs of the society that we grow up with. It also considers website usability principles. It is SIMPLE to register, to Post Items and to control your posted items. Moreover, we provide you with a quick and easy-to-use help to guide you through. At the moment, we support two languages; English and Amharic, However our goal in the future is to continue to add more native languages in Ethiopia." => ' Hulluutaraan fedhii ummata akka guutuu yaadun website hojjatame dha, Itti fayyadamuf salphaa hoggaa ta’u haala barbaaddanin itti fayyadamu ni dandeessu. Qabeenya galchuf yoo yaaddana dursa galmaa’u qabdu. Galmeessicha haala salphaan raawwachu ni dandeessu. Qabeenya galchuufis ta’e qabeenya galchitan to’achuuf baayyee salphaa dha. Akkaataa itti fayyadamaa tarreeffaman yoo isin barbaachise odeefannoo isaa jedhu keessa bal’dhiinan ni argattu. ',
    "You can post items from ANY REGIONS in Ethiopia. To contact owner of the item, you can use email or phone calls to make your own one-to-one deal directly without the involvement of us or other third party." => ' Yeroo ammati websiticha aafaan Ingiliffaan, Amaafriffaan, Oromiffaan, Tigiriffaan, Somaaliffaan, Sidaamiffaan, Guraagiffaanii fi Wallaayitiffaan ilaalu kan dandeessaan gta’u illdee gara fulduratti afaan saboota adda addaa itti daballa. Namootni naannoo hunda keessa jiraatan weebsitichaatti fayyadamu ni danda’u. Abbaa qabeenyichaaf dhaamsa ergun ykn bilbila bilbiluun waa’ee qabeenyicha osoo eenyuu iyyuu gidduu hin galin marii’achu ni dandeessu. ',
    "We take our users very seriously and attend their concern with the highest priority, therefore on hulutera, we have created a communication channel between us and our users in order to handle any concerns in using the website. Here are some tips," => ' Hulluutaraan nageenya odeefannoo qabeenyawan beeksiistaniif waan isaaf danda’ame hunda ni godha. Websiticha ilaalchisee gaaffiilee gosa kamiyyuu nu gaafachu ni dandeessu. Gaaffiilee keessaniif ariitiidhaan deebii ni kennina. Kanaaafuu akka nu gargaaruuf ',
    "For any compliant , improvements or other issues you can use" => ' Komiif, yaada foyyeessuuf yaada, akkasumas eeruwwan biroof kan gad qaba ',
    "For inappropriate items you can “Report” button" => ' Fakkiiwwanii fi ibsa gar malee galaniif “gabaasa” isa jedhu tuqatii ibsa. ',
    "If you need help, click" => ' Odeefannoo dabalataa yoo isin barbaachise kan armaan gadi tuqa ',
    "Finally, we are happy and proud to present hulutera to all Ethiopians and it is up to you to use it as much as you want. hulutera is FREE!" => ' Dhuma irratti Hulluutaraa lammiiwwan Itiyoophiyaatiif yammuu ifaa goonu fedhii guutuu fi gammachuu onnee irraa madden hoggaa ta’u websitichatati akka itti fayyadamtan kabajan isin gaafanna. ',
    "About Us" => ' Waa’ee keenya ',
    "Terms and Conditions" => ' Dmabii ittiin bulmaataa ',
    "terms and conditions text" => ' Baga nagaan gara hulutera.com dhuftan. hulutera.com qabeenyummaan isaa kan Hulluutaraa dha. Yammuu website keenya daawwatanii fi odeeffannoowwanis ta’ean wantootaa websit keenya irra jiran kamiyuutti yammuu itti fayyadamtan dambii ittiin bulmaataa armaan gadi sirriitti hunattanii fi irratti waliigaltanitu, (Waliigaltichas waliigaltee itti fayyadama jenne ibsina).

    Waliigalteen kun Caamsa guyyaa 8 bara 2012 irraa eegalee hojiirra oole jira.

    Waliigaltee itti fayyadama kana yeroo kamiyyuutti jijjiiruf mirga qabna. Wayiti jijjiiramutti website keenya irratti ni ibsina. Waliigaltee itti fayyadama dubbisuunis ta’e jijjiricha ilaaluf itti gaafatamumaan kan itti fayyadama dha.

    Akkaataa itti fayyadamaa fi naamusa itti gaafatamummaa qabu

    Website keenya yammuu daawwatanii fi odeefannoowwan website keenya irra jiranis ta’e wantoota kamiyyuutti yammuu fayyadamtan, websiticha irra kan argattan kalattiidhaanis ta’e al kallattiin bifa dambii waliigaltee itti fayyadama, seeroota hojiirra jiran, akkasumsa muxannoowwan ykn qajeelfamoota online hin cabsiinen akka ta’e irratti waliiglatanitu.

    Kanaafuu waan armaan gadi hubadha. ?

    A) wayiti websitichatti fayyadamtan odeefannoo waa’ee mataa keessanii galchuun isin irraa eegamu ni danda’a. Odeefannon galchitan sirrii fi dhugaa ta’usaaf irratti waliigaltanitu.
    B) Hulluutaraa irratti fayyadamaa taatanii galmooftanii yoo ta’e odeefannoowwan keesan ittiin galtan icciitiidhan qabachuun itti gaafatamummaan kan keesan dha. Hojiiwwan kamiyyuu odeefannoowwan keessan ittiin galaniin taasifaman kamiyyuuf itti gaafatamummaan kan keessan dha.
    C) Daandiiwwan itti fayyadamaa nuti dhiyeessine alatti websitichaatti fayyadamun gonkuma dhorka dha. Kana raawwachufis waliigalani jirtu.
    D) Gochi websitichi akka hin hojjanne ykn akka addaan citu godhu raawwachuun gonkuma dhorka dha.
    E) Wantoota websiticha irra jiran kamiyyuu koopii gochoon, heeddumeessuun, gurguruun ykn jijjiiruun gonkuma dhorka dha.
    F) Isin daawwannaa hin hayyamamnen kan ka’en rakkoo gosa kamiyyuu kalattiiodhaanis ta’e al kalattiidhaan yoo uumame itti gaafatamummaan guutuumman guutuutti kan keessani dha.
    G) Nuti odeefannoowan kamiyyuu dursine hin qulqulleesinu. Kanaafuu odeefannoowanis ta;e kanneen biro isin galchitaniif kan itti gaafatamtan isin dha. Odeefannoowwanis ta’e kanneen biroo galchitan kamiyyuu fallaa kanneen armaan gadi akka hin taane irratti waligaltanitu. ?
    i) Seera ala kan taane, kan sodaachisu, waan hamiilee nama biraa turu, waan nama dogongorsiisu, waan fokkataa ta’e, waan sanyii ta’e
    ii) hayyama daldalaa, mirga paanteentii fi koopiiraayitii qaama biraa kan hin tuqne
    iii) beeksisa hin hayyamamne
    iv) Nama ykn wqaama biraa dha jedhamanii argamu


    Fallaa Waliigaltee itti fayyadama jenne kan yaadne barreefama ykn fakkii kamiyyuu itti fayyadamaa dhaan gale kana baasuuf mirga qabna. Dhimma baasuu kana raawwachuf lafarra harkiifachuu ykn dogongoora uumamuuf kan itti gaafatamu barreefama ykn fakkii kan kan galche itti fayyadama qofaa dha.

    Daangaa wabjummaa

    Akkuma ibsame odeefannoowan Hulluutaraa irratti mul’atan kan dhiyaatan itti fayyadamaa matu ma isaatiin kan gale dha.
    Kanaafuu
    i) ykn Itti fayyadaman hundi waan barbaade hunda ni argata jennee wabummaa hin kenninu.
    ii) Qabeenyawwan mul’atan dogongoora irraa bilisa dha, Haala baayyee wiyitawaa fi kan addaan hin ciccitne dha jenne wabummaa hin kenninu..
    iii) Odefannoowwan websiticha irratti argaman guutummaa gjuutuutti sirrii dha jenne wabummaa hin kenninu.

    Kooppii raayitii (Copyright) fi mallattoo daldalaa

    Wantoonni kamiyyuu jechunis barreeffamoota, fakkiiwwan, maqaa websiyitichaa, loogoo NHjulluutaraa irratti mul’atan akkasumas qabeenyaawwan kkf qabeenyummaan isaanii kan Hulllutaraa dha. Qabkiyyeewan kanatti gochaawan malummaa hin qabne kamifuu beekmati Hulluutaraa alatti itti fayyadamun gonkuma dhorka dha.

    ykn waliigaltee ittiin fayyadamu addaan k,utuu

    Gocha kamiyyuu wal.iigaltee itti fayyadama cabsu yoo raawwattan akkuma haala isaatti akk,a itti hin tajaajilamnef gar tokkee ykn guutummaa gjuutuutti kisin dhorku ak,ka dandeenyu irratti waliigaltani jirtu. Dhorkaa kanas isin beekskisun ykn osoo isin hin beeksiisni hojiirra oolchu akka dandeenyu irratti waligaltani jirtu. Gocha yakkan shakkamu ykn waliindhahu ykn nama biraa dubbiif battalumagtti dhorka mirga itti fayyadamuma hordoofsiisa. Akkasumas zkkuma haala isaatti qaamoolee nageenyafa beeeksiisuf itti gaafatamummaa qabnu ni baana. Yammuu mirga itti fayyadamummaa keessan yeroo murtaa’ef yjn hanga dhummagtti addaan kutnuodeefannoo isin galchitan hunda dhabamsiisuuf mirga qabna. Hulluutaraa biyyootii adduunyaa heedduuttu fahyyadamun kan dhoggaa ta’u kan to’atu anda’amu nu dha.

    Dambii ittiin bulmaata kana irratti gaaffkii ykn yaada gosa kamiyyuu yoo qabattan info@hulutera.com irratti nuuf barreessa. Yookaan nu gaafachu barbaaddu isa jedhutti fayyadamun nu argadha. ',
    "about us on footer text" => ' Hulluutaraan website bitaa fi gurguurtaa kafaltii malee kan itti fayyadamtan hoggaa ta’u websiticha irra qabeenyawwan adda addaa bitachuu, gurguuruu fi kireesssuu ni dandeessu. Qaabeenyawwaan faayidaaa irra oolanis ta’e hin oole websiticha irra argacnhu ni dandeessu. Hulluutaraan yeroo ammatti Itiyoophiyaa qofaaf ta’u illee gara fuulduratti baha Afriikaatti babbal’dhiisuuf kaaayyoo qabatee sochoo’u irratti argama. Website keenya irratti mi’a xiqqaa dhuunfaan itti fayyadama irraa hanga mi’a guddaatti bituu fi gurguuruu ni dandeessju. Yeroo ammatti Hulluutaraa irraa kanneen armaan gadi bituu fi gurguuruu ni dandeessu. Konkolaataa, mana, kompiyuutarii, bilbila, kannen nama keessagtti itti tajaajilaman eeleektirooniksii fi kkf dha. <a href="../includes/template.proxy.php?type=help&lan=ao" style="color:#97caf0;font-weight:bold"> Kana </a>tuqatii odeefannoo dabalataa argachuu ni dandeessu. ',
    "ABOUT US" => ' Odeefannoo waa’ee keenyaa ',
    "INFORMATION" => ' Odeefannoo ',
    "Privacy Policy" => ' Qajeelfama dhuunfaa ',
    "FOLLOW US" => ' Nu hodoofaa ',
    "facebook" => 'acebook ',
    "twitter" => 'witter ',
    "pintrest" => 'Interest ',
    "youtube" => 'YouTube ',
    "2020 hulutera. All Rights Reserved." => ' Hulluutaraan mirgi isaa bara 2012 tti seeran kan eegame dha. ',
    "hulutera" => ' Hulluutaraa ',
    "missing email msg" => ' Maaloo teesssoo imeeli galchaa. ',
    "missing password msg" => ' Maaloo jecha kicciitii galchaa ',
    "missing e-mail and password msg" => ' Maaloo teesssoo imeelii keessani fi password keessan galchaa. ',
    "invalid email msg" => ' Imeeliin galchitan sirrii miti. ',
    "Incorrect e-mail or password msg" => ' Imeeliichi ykn jechi icciitiichaa sirrii miti. ',
    "non-existing email msg" => ' Teessoon imeelii isin galchitan maamila galmaa’e miti. ',
    "email already exists msg" => ' Imeelichi kan qabame waan ta’eef kan biraa galcha. ',
    "Forgot your password" => ' Password keessanii ni irrafattani? ',
    "delete" => ' Balleessuu ',
    "follow" => ' Hordoofuu ',
    "unfollow" => ' Hordoofuudhabuu ',
    "all msg" => ' Hunda ',
    "read" => ' Kan dubbiifame ',
    "unread" => ' Kan hin galmoofne ',
    "following" => ' Kan ani hordoofu ',
    "from" => ' Irraa ',
    "subject" => ' Mata duree ',
    "date" => ' Guyyaa ',
    "purpose" => ' Sababii ',
    "msg sent" => ' Dhaamsichi haala milkii qabun ergame jira. ',
    "reply" => ' Deebisa ',
    "first page" => ' Fula isa jalqabaa ',
    "previous" => ' Fuula isa durii ',
    "next" => ' Fuula itti aanu ',
    "last page" => ' ykn Fuula isa dhumaa ',
    "my activities" => ' Waan ani gochuu danda’u ',
    "my page" => ' Fuula koo ',
    "my profile" => ' Galmee dhunfaa koo ',
    "my page header" => ' Baga nagaan gara fuula koo dhuftan ',
    "my-page msg" => ' Haala mirkaneessaa qabeenyaa fuula <a class="alert-link" href="../../includes/template.proxy.php?type=contact&lan=ao"> kana </a>irratti galchitanii fi odeefannoo keessan geeddaru ni dandeessu. Odeefannoo irraa caalaa argachuuf <a href="../includes/upload.php?lan=ao">kana</a> tuqxanii argachu ni dandeessu. ',
    "my-page msg2" => ' Qabeenyawwan asi irratti galchitaan haala irratti argaman mirkanessu ni dandeessu. Jechuunis wantoota mul’atan, waan eeggachaa jirta, qabeenyawwan gabaasni irratti dhiyaate ykn badan ni dabalata. Jalaa tuqa itti fufa. ',
    "my-page msg3" => ' Asitti waa’ee odeefannoo dhuunfaa keessanii jijjiirjuu, sirreessuu ni dandeessju. Jechuunis jecha icciitii, maqaa ittiin fayyadamtan, karaa ittiin argamtan, kkf ni dabalata. Jala tuqa itti fufa. ',
    "to my page" => ' Gara fuul kootti ',
    "to my items" => ' Gara qabeenyawwan kootti ',
    "to admin panel" => ' Gara bulchiinsa qabeenyaatti ',
    "admin panel" => ' Bulchiinsa qabeenyaa ',
    "to my profile" => ' Galmee dhuunfaa koo ',
    "active" => ' Kan ilaalamu',
    "pending" => ' Eeggataa ',
    "all pending" => ' Eeggataa waligaltee ',
    "reported" => ' Gabaasaa kan godhame ',
    "deleted" => ' Kan bade ',
    "messages" => ' Dhaamsawwan ',
    "admin-panel" => ' Gabatee to’annoo ',
    "upload" => ' Qabeenya galchuu ',
    "choose item to upload" => ' Qabeenya galchuu barbaaddan filadhaa ',
    "profile edit" => ' odeefannoo sirreessuu ',
    "profile" => ' Odeefannoo sirreessuuf ',
    "personal info" => ' Odeefannoo dhuunfaaa ',
    "close account" => ' Akaawuntii cufuuf ',
    "acc summary" => ' walumagalltti odeefannoo kaawuntii ',
    "profile change succ" => ' Odeefannoon keessan sirriitti jijjiirameera. ',
    "address" => ' Teessooo ',
    "close" => ' Cufuu ',
    "report" => ' Gabaasa ',
    "new" => ' Isa haaraa ',
    "old" => ' Isa durii ',
    "change password" => ' Password jijjiiruu ',
    "current password" => ' Password isa ammaa ',
    "new password" => ' Password isa haaraa ',
    "enter the new password" => ' Jec ha iciitii isa haaraa galchaa ',
    "repeat new password" => ' Password isa haaraa irra debiin ',
    "enter repeated new password" => ' Password isa haaraa irra deebisatti galchaa ',
    "close acc msg part1" => ' Nu wajjinb turudhabu keessanin baayyee gaddina. Garuu yammuu kana gootan gara Hulluutaraatti akaawuntii haaraadhaan malee galu hin dandeessan, Kana malees qabeenyawwan maqaa keessaniin galchitan quwaatii odeefannoo keenya irra ni haqamu. ',
    "close acc msg part2" => ' Akaawuntii Hulluu keessan dhugumatti cufu barbaaddu? ',
    "email update succ" => ' imeeliin keessaa haala milka’in qabu jijjiirameera. ',
    "correct answer msg" => ' Maloo deebii keessan sirreessa. ',
    "close acc email msg part2" => ' Bu’uura miseensummaa Huluutaraa cufuuf gaaffataniin galmee tursiisu kana tuqun adeemsicha xumura. Akaawunticha cufuuf yoo isin gaafachubaattan dhaamsa admin@hulutera.com irratti nuuf erga. ',
    "close acc email msg part3" => '\n Nagaa wajjin \n Bulchiinsa Hulluutaraa \n ',
    "id" => ' Lakkoofsa addaa ',
    "your role" => ' Itti gaafatamummaan keessan ',
    "role" => ' Itti gaafatamummaa ',
    "active items" => ' Qabeenya mul’atu ',
    "pending items" => ' Qabeenya eeggataa ',
    "deleted items" => ' Qabeenya bade ',
    "change role" => ' Itti gaafatamummaa itti fayyadamumma geeddara ',
    "user" => ' Itti fayyadamaa ',
    "mod" => ' modareetii ',
    "admin" => ' Bulchiinsa ',
    "webmaster" => ' Weebmaastarii ',
    "all users" => ' Itti fayyadamtoota hunda ',
    "user list" => ' Tarreeffama itti fayyadamtoota waliigalaa ',
    "cp home txt1" => ' Akaakuu itti fayyadamaa ',
    "cp home txt2" => '

    Itti gaafatamummaa itti fayyadama ol guddisuu yoo barbaaddan, isa nu gaafachuu barbaadduu jedhutti fayyadamun nu beeksisa. Gara sadarkaa isa lammaffaa itti fayyadamatti ol guddistaniiirra caalaa itti fayyadama ta’a. Waa’ee akaakuwwan itti fayyadamaa Odeefannicha odeefannicha keessa ni argattu. ',
    "e.g ID username email" => ' Fakeenyaaf: Lakkoofsa addaa ykn imeelii ykn maqaa ittiin fayyadamtan ',
    "users page" => ' Fuula itti fayyadamtoota Hullutaraa ',
    "search user" => ' Itti fayyadamtoota barbaada. ',
    "no users found" => '
    odeefannoo galeen maamilan hin argamne.
    ',
    "My Items" => ' Qabeenyawwan koo ',
    "promp msg for uploading more items" => ' Qabeenya dabalataa galchuu yoo barbaaddan<a href="../includes/upload.php?lan=ao"> kana tuqaa...</a> ',
    "to main page prompt msg" => ' <a href="../index.php?lan=ao>Gara fuula muummeetti...</a>',
    "prompt msg for a wrong access to upload" => ' Qabeenya galchuuf dursitanii <a href="../includes/login.php?lan=ao">galu qabdu...</a>',
    "Back to Post Item" => ' Gara fuula qabeenya galchitaniitti ',
    "password change success" => ' Jechi icciitiichaa haala milka’en jijjirame jira. ',
    "contact-us succeed message" => ' Nu argachuuf yaalii waan gootaniif ni galateffanna. Dhaamsa keessaniif deebii yoo barbachise hanga danda’ametti guyyootii hojii 1-2 keessatti deebii ni kennina. ',
    "search-title" => ' Brbaaduu ',
    "search-button" => ' Haa barbaadamu ',
    "full no match msg" => ' Dhiifama. Qabeenya barbaaddan hin argamne. Irra deebi’a yaala </br>
                              akkaataa ittiin barbaadamuf odeefannoo yoo barbaaddan <a href="../includes/template.proxy.php?type=help&lan=ao"> kana </a>tuqxani ni argattu. ',
    "no match msg part1" => ' Dhiifatama. ',
    "no match msg part2" => ' isa jedhuuf qabeenyi gosa kamiyyuu hin argamne.
    ',
    "no match msg part3" => ' Akaataa ittiin barbaadamuuf odeefannoo yoo barbaaddaan <a href="../includes/template.proxy.php?type=help&lan=ao"> kana </a> tuqa ni argattu. ',
    "search res" => ' Bu’aa barbaachaa ',
    "failed search" => ' Maaloo waan barbaaddan galchaa ykn filannowwan biroo akkaata aittiin barbaadan filadha. ',
    "Send a message" => ' Abbaa qabeenyaaf dhaamsa erga. ',
    "Report Abuse" => ' Odeefannoo hin barbaachisne yoo ta’e eruu kenna. ',
    "Title" => ' Mata duree ',
    "Description" => ' Ibsa ',
    "More Detailed Info" => ' Tarreeffama caalaa galchuuf ',
    "pass-recovery success" => ' Jecha icciitii haaraa imeelii keessaniif galcha. Maaloo qajeelfama imeelicha irra jiru hordoofaa. Jecha icciittii keessan isa haaraa akaawuntii keesan keessa galuun jijjiiru ni dandeessu. ',
    "account activated message" => ' Akaawuntiin keessan hojiira waan oolef galuu ni dandeessu.
    Fuula akaawuntii keessan keessaa odeefannoo jijjiiruu, qabeenya galchuu fi qabeenya galchitan to’achuu ni dandeessu. ',
    "registration succeded message" => ' Waan galmooftaniif isin galateffanna. Galmaa’u keessan kan agarsiisuu imeelii isiniif ergine jirra. Maaloo imeelii isiniif ergine liinkii tuqun akaawuntii keessan hojiirraa oolacha. ',
    "email adress or username has already used" => ' Maqaan ykn imeeliin itti fayyadamicha qabame jira. Kan biraa yaala. ',
    "activation key expired" => ' Akaawuntii keessan hojiirraa osoo hin oolchin turu keessaniin kan ka’e galmeessich yeroo itti darbe jira. Kanaafuu irra deebi’agalmaa’a. ',
    "account termination message" => ' Akaawuntii Hullluutaraa cufu ilaalchisee imeelii keessan irratti odeefannooisiniif ergine jirra. Maalooqajeelfama imeelicha irra jiru hordoofaa. ',
    "Invalid" => ' Kan dhorkame ',
    "unknown" => ' Hin beekamu ',
    "unlisted" => ' Hin tarreefamne. ',
    "today" => ' Har’a ',
    "yesterday" => ' Kaleessa ',
    "noImage" => ' Suuran hin jiru ',
    "item specification" => ' Tarreefama addaatti ',
    "Enter your message" => ' Dhaamsicha asi irratti barreessa.... ',
    "contact owner" => ' Abbaa qabeenyichaf dhaamsa erga. ',
    "You forgort to enter your name, email address and Message" => ' Maqaa keessan, imeelii keessanii fi dhaamsa keessan galchu qabdu. ',
    "Login" => ' Seena ',
    "Logout" => ' Bahu ',
    "e.g" => ' Fakeenyaf ',
    "MENU" => ' Baafata ',
    "Choose Item" => ' Qabeenya filadha ',
    "Choose City" => ' Magaalaa filadhaa ',
    "All Cities" => ' Magaaloota hundatti ',
    "select city from map" => ' Kaartaa irraa magaalaa filadhaa ',
    "Choose" => 'filadhaa',
    "help" => ' Odeefannoo ',
    "or" => ' Yookaan ',
    "latest items" => ' Qabeenyawwan yeroo dhiyoo galan ',
    "latest items message" => ' Qabeenyawwan yeroo dhiyoo galan ilaaluf kan armaan gadi tuqun ilaalu ni dandeessu. ',
    "prompt" => ' Dhaamsa ',
    "watch clip" => ' Odeefannoo dabalataaf vidiiyoo ilaala.',
    //new ones
    'no items to show' => '<div id="spanColumnXamharic">ይቅርታ!የሚታይ ምንም ንብረት የለም</div>',
    'remove' => 'ለማጥፋት',
    'item remove confirmation msg' => 'ይህንን ንብረትዎን ለማስተዋወቅ ያስገቡትን ማስታወቂያ ለማጥፋት ወስነዋል?',
    'yes' => 'ወስኛለሁ',
    'no' => 'አልወሰንኩም'
);

$city_lang_arr = array(
    "000" => 'Magaalaa filadhaa   ',
    "All" => ' Magaaloota hunda    ',
    "Addis Ababa" => ' Finfinnee     ',
    "Adama" => ' Adaamaa    ',
    "Bahir Dar" => ' Baahirdaar    ',
    "Mekele" => ' Maqalee  ',
    "Hawassa" => ' Hawaasaa  ',
    "Dire Dawa" => ' Dirreedawaa     ',
    "Gonder" => ' Gondor   ',
    "Dessie" => ' Dassee ',
    "Jimma" => ' Jimmaa   ',
    "Jijiga" => ' Jijjiigaa  ',
    "Shashemene" => ' Shaashamannee ',
    "Bishoftu" => ' Bishooftuu ',
    "Sodo" => ' Soddoo ',
    "Arba Minch" => ' Arbaamincii  ',
    "Hosaena" => ' Hosaa’inaa     ',
    "Harar" => ' Harar    ',
    "Dila" => ' Diillaa    ',
    "Nekemte" => ' Naqamtee  ',
    "Debre Birhan" => ' Dabrabirhaan   ',
    "Asella" => ' Asallaa   ',
    "Ambo" => ' Amboo ',
    "Gambela" => ' Gambeelaa    ',
    "Axum" => ' Aksuum    ',
    "Asaita" => ' Assaayitaa  ',
    "Asossa" => ' Asoosaa    ',
    "Debre Markos" => ' Dabramaarqoos    ',
    "Adigrat" => ' Addiigraat  ',
    "Other" => 'Kan biroo'
);

$addis_sub_city = array(
    "Choose sub-city" => ' Kutaa magaalaa filadhaa ',
    "Addis ketema" => ' Addis Katamaa  ',
    "Akaki Kality" => ' Aqaaqii Qaaliittii  ',
    "Arada" => ' Araadaa ',
    "Bole" => ' Boolee  ',
    "Gullele" => ' Gullalee  ',
    "Kirkos" => ' Qiqoos  ',
    "Kolfe Keraniyo" => ' Kolfee Qaraaniyoo  ',
    "Lideta" => ' Lidataa  ',
    "Nifassilk Lafto" => ' N/S/Laaftoo  ',
    "Yeka" => ' Yekkaa'
);

$item_lang_arr = array(
    "000" => 'Qabeenya filadhaa   ',
    "All" => ' Qabeenyawwan hunda    ',
    "car" => ' Konkolaataa     ',
    "house" => ' Mana     ',
    "computer" => ' Kompiyutara    ',
    "phone" => ' Bilbila     ',
    "electronic" => ' Eeeleektironiksii     ',
    "household" => ' Meeshaalee mana keessaa   ',
    "other" => ' Kan biroo'
);

$upload_specific_array = [
    'car' => [
        'Uploading' => 'Unkaa konkoolaataa ittiin galchan',
        'idCategory'        => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                "Bus" => ' Otonusii  ',
                "Compact Car" => ' Konkooltaa xiqqaa kan maatii   ',
                "Converitble" => ' Baaxiin isaa kan banamu   ',
                "Full Size Van" => ' Vaan     ',
                "Hatchback" => ' Haachibaakii  ',
                "Heavy Machinery" => ' Mashiinii guddaa     ',
                "Luxury Car" => ' Konkoolaata bashnnanaa    ',
                "Minibus" => ' Minibaasii    ',
                "Pickup" => ' Piikaappii     ',
                "Small Car" => ' Konkolaataa xiqqaa kan manaa ',
                "Sport Car" => ' Konkoolaataa kankoolaataa kan ispoortii   ',
                "Station Wagon" => ' Konkoolaataa agidduugaleessaa kan manaa   ',
                "SUV" => ' SUV     ',
                "Taxi" => ' Taaksii     ',
                "Truck" => ' Konkooltaa fe’umsaa   ',
                "Other" => '—Hin tarreefamne--     ',
            ]
        ],
        'fieldMake'        => [
            0 => 'Biyyaa itti hojjatame',
            1 => 'Biyyaa itti hojjatame filadhaa',
            2 => [
                "apollo" => "Apollo",
                "aston-martin" => "Aston Martin",
                "audi" => "Audi",
                "bentley" => "Bentley",
                "bmw" => "BMW",
                "buick" => "Buick",
                "cadillac" => "Cadillac",
                "chevrolet" => "Chevrolet",
                "chevrolet-truck" => "Chevrolet",
                "chrysler" => "Chrysler",
                "citroën" => "Citroën",
                "dodge" => "Dodge",
                "deawoo" => "Deawoo",
                "ferrari" => "Ferrari",
                "fiat" => "Fiat",
                "fisker" => "Fisker",
                "ford" => "Ford",
                "freightliner" => "Freightliner",
                "gmc" => "GMC",
                "honda" => "Honda",
                "holland-car" => "Holland Car",
                "hyundai" => "Hyundai",
                "infiniti" => "Infiniti",
                "isuzu" => "Isuzu",
                "iveco" => "Iveco",
                "jaguar" => "Jaguar",
                "jeep" => "Jeep",
                "kia" => "Kia",
                "lamborghini" => "Lamborghini",
                "land-rover" => "Land Rover",
                "lexus" => "Lexus",
                "lifan" => "Lifan",
                "lincoln" => "Lincoln",
                "lotus" => "Lotus",
                "maserati" => "Maserati",
                "maybach" => "Maybach",
                "mazda" => "Mazda",
                "mercedes-benz" => "Mercedes-Benz",
                "mini" => "Mini",
                "mitsubishi" => "Mitsubishi",
                "nissan" => "Nissan",
                "opel" => "Opel",
                "peugeot" => "Peugeot",
                "porsche" => "Porsche",
                "ram" => "Ram",
                "renault" => "Renault",
                "rolls-royce" => "Rolls Royce",
                "saab" => "Saab",
                "scion" => "Scion",
                "seat" => "SEAT",
                "sinotruck" => "Sinotruck",
                "smart" => "Smart",
                "subaru" => "Subaru",
                "suzuki" => "Suzuki",
                "tesla" => "Tesla",
                "toyota" => "Toyota",
                "volkswagen" => "Volkswagen",
                "volvo" => "Volvo",
                'unlisted' => '-- አልተዘረዘረም --'
            ]
        ],
        'fieldModel'       => [
            0 => 'Moodeelii',
            1 => 'Moodeelii asitti barreessaa'
        ],
        'fieldModelYear'   => [
            0 => 'Bara itti hojjatame',
            1 => 'Bara itti hojjatame filadhaa',
            2 => [
                '1970' => '1970 - 1979',
                '1960' => '1960 - 1969',
                '1950' => '1950 - 1959',
                '1940' => 'Bara 1950 dura',
                '000' => '--Hin tarreefamne--'
            ],
            3 => [
                '2020' => '2020 - 2021',
                '2015' => '2015 - 2019',
                '2010' => '2010 - 2014',
                '2005' => '2005 - 2009',
                '2000' => '2000 - 2004',
                '1990' => '1990 - 1999',
                '1980' => '1980 - 1989',
                '1970' => '1970 - 1979',
                '1960' => '1960 - 1969',
                '1950' => '1950 - 1959',
                '1940' => 'Bara 1950 dura'
            ]
        ],
        'fieldGearType'   => [
            0 => 'Akaakuu maarshii (Giirbooksii)',
            1 => 'Akaakuu maarshii filadhaa',
            2 => [
                'Manual' => 'Maanuwaalaii',
                'Automatic' => 'Otomaatikii',
                'Semi-automatic'  => 'Gar tokkee Otomaatikii'
            ]
        ],
        'fieldFuelType'   => [
            0 => 'Akaakuu boba’aa',
            1 => 'Akaakuu boba’aa filadhaa',
            2 => [
                'Bensine' => 'Beenziilan',
                'Diesel'  => 'Naafxaa',
                'Electric' => 'Eeleektiriikiin',
                'Bio-gas' => 'Baayoogaazii',
                'Hybrid' => 'Haayibiriidii'
            ]
        ],
        'fieldMilage' => [
            0 => 'Hanga imale (Km dhaan)',
            1 => 'Hanga imale filadhaa',
            2 => ['unlisted' => '--Hin tarreefamne--']
        ],
        'fieldNoOfSeat' => [
            0 => 'Baay’ina barbcumaa',
            1 => 'Baay’ina barcuma filadhaa',
            2 => [
                '101' => '100 ol',
                '000' => '--Hin tarreeffamne--'
            ]
        ],
        'fieldColor'       => [
            0 => 'Haalluu',
            1 => 'Haalluu filadhaa',
            2 => [
                "red" => ' Diimaa    ',
                "green" => ' Magarisa     ',
                "blue" => ' culuqqisa ',
                "yellow" => ' Keelloo ',
                "black" => ' Gurraaacha    ',
                "white" => ' Adii     ',
                "gray" => ' daalacha    ',
                "silver" => ' Silvarii    ',
                "liver" => ' Waan akka tulluu     ',
                "brown" => ' Bunnii    ',
                "unlisted" => '—Hin tarreefamne--     ',
            ]
        ]
    ],
    'house' => [
        'Uploading' => 'Unka mana, lafa irratti galchan',
        'idCategory'        => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                'Commercial' => 'Kan daldalaa',
                'Land' => 'Lafa duwwaa',
                'Condominium' => 'Kondominiyamii',
                'Residential' => 'Mana dhuunfaa',
                'Other' => '--Hin tarreefamne--'
            ]
        ],
        'fieldLotSize' => [
            0 => 'Bal’dhiina lafaa',
            1 => 'Bal’dhiina lafaa Kaaree meetiran asi asitti barreessaa',
            2 => [],
        ],
        'fieldKebele'        => [
            0 => 'Ganda',
            1 => 'Ganda filadhaa',
        ],
        'fieldWereda'        => [
            0 => 'Aanaa',
            1 => 'Aanaa filadhaa',
        ],
        'fieldBuildYear'   => [
            0 => 'Bara itti hojjatame',
            1 => 'Bara itti hojjatame filadhaa',
            2 => [
                '1970' => '1970 - 1980',
                '1960' => '1950 - 1960',
                '1950' => '1940 - 1950',
                '1940' => 'Bara 1940 dura',
                'unlisted' => '--Hin tarreefamne--'
            ],
            3 => [
                '2011' => 'ከ 2010 - 2013',
                '2010' => 'ከ 2005 - 2009',
                '2000' => 'ከ 2000 - 2004',
                '1995' => 'ከ 1995 - 1999',
                '1990' => 'ከ 1990 - 1999',
                '1980' => 'ከ 1980 - 1989',
                '1970' => 'ከ 1970 - 1979',
                '1960' => 'ከ 1960 - 1969',
                '1950' => 'ከ 1950 - 1959',
                '1940' => 'Bara 1940 dura'
            ]
        ],
        'fieldBathroom'   => [
            0 => 'Baay’ina 196 9dhiqataa qabu',
            1 => 'Mana itti dhiqatan filadhaa',
        ],
        'fieldToilet'   => [
            "0" => 'Baay’ina mana finii',
            "1" => 'Baay’ina filadhaa',
            "2" => '100 ol',
            "3" => 'Baay’ina mana fincaanii guddaa',
        ],
        'fieldNrBedroom' => [
            "0" => ' Baay’ina mana cisiichaa ',
            "1" => ' Baay’ina filadhaa   ',
            "2" => ' 100 ol    ',
            "3" => ' Baay’ina mana cisiichaa guddaa   '
        ],
        'fieldWater' => [
            0 => 'Manichi bishaan qabaa?',
            1 => 'Bishaan filadhaa',
            2 => [
                'Yes' => 'Qaba',
                'No' => 'Hin qabu',
                'ongoing' => 'Adeemsa irra jira'
            ]
        ],
        'fieldElectricity' => [
            0 => 'Manichi humna eelektiriikii qabaa?',
            1 => 'Eeleektiriikii filadha',
            2 => [
                'Yes' => 'Qaba',
                'No' => 'Hin qabu',
                'ongoing' => 'Adeemsa irra jira'
            ]
        ]
    ],
    'computer' => [
        'Uploading' => 'Unkaa kompiyutara irra ittiin galchan',
        'idCategory'        => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                'Laptop' => 'Laptop',
                'Notebook' => 'Notebook',
                'Stationary' => 'Stationary',
                'Tablets' => 'Tablets',
                'Workstations' => 'Workstations',
                'Other' => '--Hin tarreefamne--'
            ]
        ],
        'fieldMake'        => [
            0 => 'Biyya itti hojjatame',
            1 => 'Adii  Biyya aitti hojjatame filadhaa',
            2 => [
                "acer" => "Acer",
                "alienware" => "Alienware",
                "apple" => "Apple",
                "asus" => "Asus",
                "benq" => "Benq",
                "compaq" => "Compaq",
                "cybertonpc" => "Cybertonpc",
                "cyberpower" => "Cyberpower",
                "dell" => "Dell",
                "dai-tech" => "Dai-Tech",
                "epson" => "Epson",
                "gateway" => "Gateway",
                "hp" => "HP",
                "hitachi" => "Hitachi",
                "huawei" => "Huawei",
                "ibm" => "IBM",
                "ibuypower" => "Ibuypower",
                "lenovo" => "Lenovo",
                "lg" => "LG",
                "microsoft" => "Microsoft",
                "samsung" => "Samsung",
                "sony" => "Sony",
                "toshiba" => "Toshiba",
                "vizio" => "Vizio",
                "msi" => "msi",
                "unlisted" => "-- አልተዘረዘረም --"
            ]
        ],
        'fieldModel'       => [
            0 => 'Modeelii',
            1 => 'Modeelii asitti barreessaa'
        ],
        'fieldHardDrive'   => [
            0 => 'Hard disc',
            1 => 'Hard disc filadhaa',
            2 => [
                "Under 100GB" => ' 99GB gadi    ',
                "100 - 199GB" => ' 100 - 199GGB  ',
                "200 - 299GB" => ' 200 - 299GB',
                "300 - 499GB" => ' 300 - 499GB',
                "500 - 1TB" => '500 - 1TB     ',
                "1 - 2TB" => '1 - 2TB     ',
                "2 - 3TB" => '2 - 3TB     ',
                "3 - 4TB" => '3 - 4TB     ',
                "4 - 5TB" => '4 - 5TB     ',
                "Over 5TB" => '5TB ol ',
                "unknown" => ' Hin beekamu     '
            ]
        ],
        'fieldOs'   => [
            0 => 'Sirna recycle kompiyutaraa (OS) ',
            1 => 'Sirna recycle kompiyutaraa filadhaa',
            2 => [
                "Windows" => "Windows",
                "Linux" => "Linux",
                "Unix" => "Unix",
                "Mac" => "Mac",
                'unlisted' => '--Hin tarreefamne--',
            ]
        ],
        'fieldProcessor'   => [
            0 => 'Safiisa proccesor ',
            1 => 'Safiisa processor filadhaa',
            2 => [
                "Under 1 GHz" => ' 1 GHz gadi     ',
                "1.0 - 1.49GHz" => '1.0 - 1.49GHz     ',
                "1.5 - 1.99GHz" => '1.5 - 1.99GHz     ',
                "2.0 - 2.49GHz" => '2.0 - 2.49GHz     ',
                "Over 3.0GHz" => '3.0 GHz ol  ',
                "unknown" => ' Hin beekamu   '
            ]
        ],
        'fieldRam' => [
            0 => 'Hanga RAM Memory',
            1 => 'RAM Memory filadhaa',
            2 => [
                "Under 1GB" => ' GB1 gadi     ',
                "1.0 - 1.9GB" => '1.0 - 1.9GB     ',
                "2.0 - 2.9GB" => '2.0 - 2.9GB     ',
                "3.0 - 3.9GB" => '3.0 - 3.9GB     ',
                "Over 4.0GB" => '4.0GB GHz ol     ',
                "unknown" => ' Hin beekamu '
            ]
        ]
    ],
    'phone' => [
        'Uploading' => 'Unkaa bilbila irratti galchan',
        'idCategory'        => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                "Cell Phone" => ' Bilbila mobaayilii  ',
                "Smart Phone" => ' Smart Phone    ',
                "Fixed Phone" => ' Bilbila manaa ',
                "PDA" => ' PDA ',
                "Smart Watch" => ' Sa’aatii Smart     ',
                "Phone Accessories" => ' Meeshaalee jijjiiraa bilbilaa   ',
                "Other" => '—Hiin tarreeffamne--'
            ]
        ],
        'fieldMake'        => [
            0 => 'Biyyaa itti hojjatame',
            1 => 'Biyyaa itti hojjatame filadhaa',
            2 => [
                "Alcatel" => "Alcatel",
                "asus" => "Asus",
                "Blackberry" => "Blackberry",
                "HTC" => "HTC",
                "Huawei" => "Huawei",
                "iPhone" => "iPhone",
                "LG" => "LG",
                "Motorolla" => "Motorolla",
                "Nokia" => "Nokia",
                "Samsung" => "Samsung",
                "Sanyo" => "Sanyo",
                "Siemens" => "Siemens",
                "Sony" => "Sony",
                "Sony Ericsson" => "Sony Ericsson",
                "tecno" => "Tecno",
                "T-Mobile" => "T-Mobile",
                "TANA" => "TANA",
                "Vodaphone" => "Vodaphone",
                "ZTE" => "ZTE",
                'unlisted' => '--Hin tarreeffamne--'
            ]
        ],
        'fieldModel'       => [
            0 => 'Moodeelii',
            1 => 'Moodeelii asitti barreessaa'
        ],
        'fieldCamera'   => [
            0 => 'Hanga kaameeraa bilbilaa (M.P.)',
            1 => 'Hanga kaameera filadhaa',
            2 => [
                "more than 16 MP" => ' 16 ol     ',
                "11.0 - 15.9 MP" => '11.0 - 15.9     ',
                "8.0 - 10.9 MP" => '8.0 - 10.9     ',
                "4.0 - 7.9 MP" => '4.0 - 7.9     ',
                "1.0 - 3.9 MP" => '1.0 - 3.9     ',
                "Under 1 MP" => ' 1.0 gadi     ',
                "unknown" => ' Hin beekamu   '
            ]
        ],
        'fieldOs'   => [
            0 => 'Sirna recycle sa’aatii (OS) ',
            1 => 'Sirna recycle sa’aatii filadhaa',
            2 => [
                "Windows" => "Windows",
                "Android" => "Android",
                "iOS" => "iOS",
                "Symbian" => "Symbian",
                'unlisted' => '--Hin tarreeffamne--',
            ]
        ],

    ],
    'electronic' => [
        'Uploading' => 'Unkaa eeleektirooniiksii ittiin galchaa',
        'idCategory' => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                "Camera" => ' Kaameeraa    ',
                "Refrigerator" => ' Cabbeessaa (Refrgeretor)   ',
                "Games" => ' Eeleektironiiksii ittiin taphatan ',
                "Headphone" => ' Gurraan kan itti dhageefatan  ',
                "Watch" => ' Sa’aatii harkaa, sa’aatii goorroo, kkf     ',
                "Tape Recorder" => ' Sagaaleekan ittiin waraaban  ',
                "TV" => ' TV    ',
                "unlisted" => '—Hin tarreefamne --     '
            ]
        ]
    ],
    'household' => [
        'Uploading' => 'Unkaa meeshaalee manakeessaa ittiin galchan',
        'idCategory' => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                "Furniture" => ' Barcuuma, minjaala, siree, kkf  ',
                "Kitchen Appliances" => ' Meeshaalee alawaadaa/kushinaa/ ',
                "Bathroom Appliances" => ' Meeshaalee mana baanyoo  ',
                "Home Decor" => ' Meeshaalaa mana miidhagsan ',
                "Bedroom Appliances" => ' Meeshaalee mana cisiichaa ',
                "Baby Gears" => ' Meeshaalee daa’immanif itti fayyadaman ',
                "Other" => ' Kan biraa  '
            ]
        ]
    ],
    'other' => [
        'Uploading' => 'Unkaa meeshaalee adda addaaittiin galchan',
        'idCategory' => [
            0 => 'Akaakuu',
            1 => 'Akaakuu filadhaa',
            2 => [
                "Clothes" => ' Huuccuulee ',
                "Shoes" => ' Kopheewwan ',
                "Jewellery" => ' Wantoota ittiin miidhagan ',
                "Tools" => ' Meeshaalee suphaa ',
                "Sport" => ' Meeshaalee ispoortii ',
                "Entertainment" => ' Meeshaalee bashannanaa   ',
                "Other" => '--Hin tarreefamne--'
            ]
        ]
    ],
    'common' => [
        'fieldLocation'    => [
            0 => 'Iddoo qabeenyichi itti argamu',
            1 => 'Iddoo qabeenyicha filadhaa'
        ],
        'rentOrSell' => [
            0 => 'Kireessuu moo gurguruu barbaaddu?',
            1 => 'Kiraa ykn gurguurtaa filadhaa',
            2 => [
                'fieldPriceRent' => 'Kireessuu',
                'fieldPriceSell' => 'Gurguruu',
                'both' => 'Hunda'
            ],
            3 => 'Trreedamagatii'
        ],
        'fieldPriceRent' => [
            0 => 'Gatii kiraa',
            1 => 'Gatii kiraa asitti barreessaa, 0 Hin fudhu',
            2 => 'Hin kireeffamu',
        ],
        'fieldPriceRate' => [
            0 => 'Tilmaama gatii kiraa',
            1 => 'Tilmaama gatii kiraa filadhaa',
            2 => [
                'hourly' => 'Sa’aatiitti',
                'daily' => 'Guyyaatti',
                'monthly' => 'Ji’atti',
                'yearly' => 'Waggaatti',
            ]
        ],
        'fieldPriceSell' => [
            0 => 'Gatii gurguurtaa',
            1 => 'Gatii gurguurta asitti barreessaa, 0 Hin fudhatau',
            2 => 'Hin gurguuramu',
            3 => 'Baaxii bittaa filadhaa',
            4 => 'Baaxii bittaa (Qarshii)',

            // max price list for car
            5 => [
                "6000000" => ' 5 000 000 ol    ',
                "5000000" => '5 000 000     ',
                "3000000" => '3 000 000     ',
                "1000000" => '1 000 000     ',
                "800000" => '900 000     ',
                "700000" => '700 000     ',
                "600000" => '600 000     ',
                "500000" => '500 000     ',
                "300000" => ' 300 000 gadi    '
            ],

            // max price list for house
            6 => [
                "20000001" => ' 20 000 000 ol     ',
                "20000000" => '20 000 000     ',
                "10000000" => '10 000 000     ',
                "8000000" => '8 000 000     ',
                "5000000" => '5 000 000     ',
                "3000000" => '3 000 000     ',
                "1000000" => '1 000 000     ',
                "900000" => '900 000     ',
                "700000" => '700 000     ',
                "500000" => '500 000     ',
                "300000" => ' 300 000 gadi    '
            ],

            // max price list for computer
            7 => [
                "100001" => ' 100 000 ol     ',
                "100000" => '100 000     ',
                "75000" => '75 000     ',
                "50000" => '50 000     ',
                "40000" => '40 000     ',
                "35000" => '35 000     ',
                "30000" => '30 000     ',
                "25000" => '25 000     ',
                "20000" => '20 000     ',
                "15000" => '15 000     ',
                "10000" => '10 000     ',
                "5000" => ' 5 000 gadi  '
            ],

            // max price list for phone
            8 => [
                "50001" => ' 50 000 ol     ',
                "50000" => '50 000     ',
                "40000" => '40 000     ',
                "35000" => '35 000     ',
                "30000" => '30 000     ',
                "25000" => '25 000     ',
                "20000" => '20 000     ',
                "15000" => '15 000     ',
                "10000" => '10 000     ',
                "7500" => '7 500     ',
                "5000" => '5 000     ',
                "2500" => '2 500     ',
                "1500" => ' 1 500 gadi'
            ]

        ],
        'fieldPriceCurrency' => [
            0 => 'Sharafa  ',
            1 => 'Sharafa filadhaa',
            2 => [
                'ETB' => 'Qarshii',
                'USD' => 'Doolaaraa Ameeriikaa',
            ]
        ],
        'fieldPriceNego' => [
            0 => 'Gatii irratti marii’achuun ni danda’ama?',
            1 => 'filadhaa',
            2 => [
                'Yes' => 'Gatii irratti marii’achuun ni danda’ama.',
                'No' => 'Gatii irratti marii’achuun hin danda’amu.'
            ]
        ],
        'fieldTitle' => [
            0 => 'Mata duree',
            1 => 'Mata duree asiti barreessaa'
        ],
        'marketType' => [
            'rent' => 'Kan kireeffamu',
            'sell' => 'Kan gurguuramu',
            'rent and sell' => ' Kan kireefamuu ykn kan gurguuramu'
        ],
        'Extra Info' => 'Odeefannoo dabalataa',
        'fieldContactMethod' => [
            0 => 'Karaa itti argamu',
            1 => 'Karaa itti argamu filadhaa',
            2 => [
                'phone'  => 'Bilbilan',
                'e-mail' => 'Imeeliin',
                'both'   => 'Bilbilaa fi imeelii dhaan'
            ]
        ],
        'fieldColor'       => [
            0 => 'Haalluu',
            1 => 'Haalluu filadhaa',
            2 => [
                "red" => ' Diimaa     ',
                "green" => ' Magariisaa ',
                "blue" => ' Cuquulisa  ',
                "yellow" => ' Keelloo ',
                "black" => ' Gurraachaa  ',
                "white" => ' Adii   ',
                "gray" => ' Daalcha     ',
                "silver" => ' Silvarii ',
                "liver" => ' Waan akka tullu     ',
                "brown" => ' Bunnii   ',
                "unknown" => '--Hin tarreeffamne--'
            ]
        ],
        'Choose Images here' => ' Fakkiiwwan asitti galchaa',
        'upload successful msg' => 'Tajaajila keenyati waan fayyadamtaniif ni galateffanna. Qabeenyi keessan haala milka’en gale jira. Qabeenya keessaniif odeefannoo dabalata imeelii keessan irratti isiniif ergine jirra.'
    ]
];

$abuse_type_lang_arr = [
    0 => [
        "000" => ' Akaakuu filadhaa   ',
        "Bullying" => ' Kan biraa xureessuu ',
        "Copyright" => ' Hanna mirga abbeentaa (Copy right)    ',
        "Discrimination" => ' Looguu    ',
        "Spam" => ' Kolkolee   ',
        "Identity theft" => ' Hanna eenyuummaa    ',
        "Political violence" => ' Jeequmsa siyaasaa kan uumuu    ',
        "Race violence" => ' Qabiyyee sanyii kan qabu   ',
        "Sex Abuse" => ' Miidhaa saalaa   ',
        "Sexual Content" => ' Qabiyee saala kan qabu  ',
        "Religious violence" => ' Jeequmsa amantii kan uumu  ',
        "Other" => ' Kan biraa   '
    ],
    1 => [
        'You forgort to choose the Report type' => 'Akaakuu gabaasaa filachuu iraanfattani jirtu.',
        'You successfully reported the item!' => 'Gabaasichi haala milkaa’en raawwate jira.'
    ]
];

$user_specific_array = [
    'user' =>    [
        'registeration' => 'Unkaa maamila ittiin galmeessan',
        'login' => 'Unkaa ittiin seenan',
        'contactus' => 'Unkaa yaada itti kennan',
        'activate' => 'Gibira irraa',
        'passwordRecovery' => [
            "0" => ' Unkaa yaadannoo icciitii  ',
            "1" => ' Ergaa   ',
            "2" => ' Itti fayyadamana website keenya irrati hin galmoofne.     ',
            "3" => ' Jecha icciitii yeeroodhaafitti fayyadamun galuu yoo barbaaddan walqunnaamsiisa kana tuqa.    '
        ],
        'fieldName'       => [
            "0" => ' Maqaa guutuu  ',
            "1" => ' Maqaa guutuu asitti barreessaa  ',
            "2" => ' Unkaa maqaa guutuu ittiin jijjiiran '
        ],
        'fieldUserName'       => [
            "0" => ' Maqaa ittiin fayyadamtan     ',
            "1" => ' Maqaa ittiin fayyadamtan asitti barreessaa ',
            "2" => ' Unkaa maqaa ittiin fayyadamaa ittiin jijjiiran  '
        ],
        'fieldFirstName'       => [
            "0" => ' Maqaa ',
            "1" => ' Maqaa keessan asitti barreessaa ',
            "2" => ' Unkaa maqaa gutuu ittiin jijjiiran '
        ],
        'fieldLastName'       => [
            "0" => ' Maqaa abbaa  ',
            "1" => ' Maqaa abbaa asitti barreessaa  ',
            "2" => ' Unkaa maqaa guutuu ittiin jijjiiran '
        ],
        'fieldEmail'       => [
            "0" => ' Imeelii ',
            "1" => ' Teessoo imeelii asitti barreessaa  ',
            "2" => ' Unkaa teessoo imeelii ittiin jijjiiran  '
        ],
        'fieldPhoneNr'       => [
            "0" => ' Bilbila ',
            "1" => ' Bilbila keessan asitti barreessaa ',
            "2" => ' Unkaa teesssoo lakkoofsa bilbilaa ittiin jijjiiran   '
        ],
        'fieldPassword'       => [
            "0" => ' Jecha icciitii  ',
            "1" => ' Jecha icciitii barreessaa  ',
            "2" => ' jecha icciitii agarsiisi. ',
            "3" => ' Unkaa jecha icciitii ittiin jijjiiran '
        ],
        'fieldPasswordRepeat'       => [
            "0" => ' Jecha icciitii irra deebi’aa ',
            "1" => ' Jecha icciitii irra deebi’aa barreessaa  ',
            "2" => ' Jecha icciitii agarsiisi.    ',
            "3" => ' Unkaa jecha icciitii ittiin jijjiiran '
        ],
        'fieldPasswordRepeat2'       => [
            "0" => ' Jecha icciitii irra deebi’aa ',
            "1" => ' Jecha icciitii irraa deebi’aa barreessaa ',
            "2" => ' Jecha icciitii agarsiisi. ',
            "3" => ' Unkaa jecha icciitii ittiin jijjiiran '
        ],
        'fieldContactMethod' => [
            0 => 'Karaa ittiin argamu',
            1 => 'Karaa ittiin argamu filadhaa',
            2 => [
                'phone'  => 'Bilbilan',
                'e-mail' => 'Imeeliin',
                'both'   => 'Bilbilaa fi imeeliin'
            ],
            3 => 'Unkaa karaa ittiin argan kan ittiin jijjiiran'
        ],
        'fieldTermAndCondition'       => [
            0 => 'Dambii ittiin bulmaataa fi qajeelfama dhuunfaa',
            1 => 'Damabii ittiin bulmaaticha irratti waliigalu keessaniif filadhaa',
            2 => [
                'Yes' => 'Eeyyee! Dmabii ittiin bulmaatichaaf  fi qajeelfama dhuunfaa dubbiisee irartti waliigaleen jira.'
            ],
            3 => [
                'message' => '<strong> Dmabii ittiin bulmaataa </strong> dubbiisuuf tuqa'
            ]
        ],
        'fieldPrivacyPolicy' => [
            0 => [
                'message' => '<strong> Qajeelfama dhuunfaa </strong> dubbiisuuf asi tuqaa'
            ]
        ],
        'fieldCompany'       => [
            0 => 'Maqaa dhaabbataa',
            1 => 'Maqaa dhaabbataa asitti barreessaa yoo ta’ubaate iddoo duwwaa dhiisaa'
        ],
        'fieldSubject'       => [
            0 => 'Mata duree',
            1 => 'Mata dure asitti barreessaa'
        ],
        'fieldMessage'       => [
            0 => 'Dhaamsa',
            1 => 'Dhaamsa asitti barreessaa'
        ],
        'fieldPurpose'       => [
            0 => 'Sababii itti nu hasoofsiisuu barbaaddaniif',
            1 => 'Sababii itti nu haasoofsiisuf barbaaddaniif filadhaa',
            2 => [
                1 => 'Yaada waliigalaa',
                2 => 'Qabeenyan galche argachuu hin dandeenye',
                3 => 'Qabeenyi ani galchee hin mirkanoofne',
                4 => 'qabeenyi ani galche active miti',
                5 => 'Beeeksiisuf yammuun yaalu rakkoon teekniikaa na muddate jira.',
                6 => 'Fakkiin sirriitti hin mulatu',
                7 => 'Gabaasa gochaa waliindhawu gochuun barbaada',
                8 => 'Hulluutaraaf yaada kennuun barbaada',
            ]
        ],
    ],
    'message' => [
        'activation' => [
            'subject' => 'Mirkaneessaa galmeessa fi ittiin jalqabsiisan',
            'body' => 'irratti Waan nu filattaniifi waan galmooftanii ni galateffanna. Galmeessichaa xumuruu fi tajaajila Hulluutaraatti fayyadamuu eegaluuf appiilicatiin kana tuqa.',
        ],
        'password-recovery' => [
            'subject' => 'Jecha icciitii haaraa galchaa',
            'body' => [
                0 => "Kabajamoo maamile keenyaa, \n jechaa iciittii haaraa jijjiiruuf gaaffatani ture. \n Bu’uura kanaan appilication jala jiru tuquu fi jecha icciitii isa haaraati fayyadamun eega galtan booda isa jecha iciitii jijjiiruf jedhutti fayyadamun jijjiiru ni dandeessu. Dhaamsi kun kan ergame mata jalatti (awutomaatikii) kan ta’en karaa dhaamsa ittiin erganiin waan ta’eef jecha icciitiichaa geeddaruufkan gaafattaan yoo isin ta’ubaattan teessoo imeelii kana irratti nuuf erga. admin@hulutera.com\n",
                1 => [
                    "\nKomiif:",
                    "\njechaa iciittii:",
                    "\nSincerely,\nThe hulutera Admin<br>"
                ],
            ]
        ],
        'edit-profile' => [
            'subject' => 'Mirkaneessaa galmee dhuunfaa Hulluutaraa jijjiiruu',
            'body' => [
                0 => 'Kabajamoo maamila keenyaa, <br> Odeefannoo dhuunfaa keessanii Hulluutaraa irra qabdan geeddaruf nu gaafattani ture. Bu’uura odeefannoo kanaan sirraa’usaaf mirkaneessa akka ta’u dhaamsi kun isiniif ergame jira. <br> Nagaa wajjin <br> Bulchiinsa Hulluutaraa<br>'
            ]
        ],
        'contact-us' => [
            'subject' => 'Nu argachuuf yaalii gootaniif ni galatefanna.',
            'body' => [
                0 => 'Dhaamsi keessan nu gahe jira. Waa’ee nuuf bareesitaniif ni galatefanna. Gaaffin keessan ariifachiisaa yoo ta’e maaloo lakkoofsa bilbilaa kanaa olitti tarrefaman irratti bilbiluun hojjattoota keenya keessa isa tokko haasofsiisaa. Kanachii immoo ariitii danda’amen imeeliidhaan deebii ni kennina. <br> Nagaa wajjin <br> Bulchiinsa Yekaatoomar <br>'
            ]
        ],
        'account-deactivation' => [
            'subject' => 'Your account have been deactivate',
            'body' => [
                0 => 'We have found problem with your account not complying to our Term& conditions
                      hence your account have been deactivated and your uploaded items will not be
                      advertised. Please reach out to us for addtional through contact us.
                      <br>Sincerely,<br>The hulutera Admin<br>'
            ]
        ],
        'account-closed' => [
            'subject' => 'Your account have been closed',
            'body' => [
                0 => 'We have closed your account! Have a good one!
                      <br>Sincerely,<br>The hulutera Admin<br>'
            ]
        ]
    ]
];

$validate_specific_array = [
    0 => ' Filatamu qaba. Irra deebi’a yaala!</br>',
    1 => ' Guutamu qaba, Irraa deebi’a yaala!</br>',
    2 => [
        'fileuploader-list-files' => ' Yoo xinnaate fakkiin tooko tuqamu qaba. Irra deebi’a yaala!',
        'rentOrSell' => ' Kiraan ykn gurguurtan filatamu qaba. Irra deebi’a yaala!',
        'number' => ' Sabsii (input) dhorkame! Barreessuun kan danda’amu lakkoofsaa zeeroo (0) ol jiru (lakkoofsan) qofaa dha. Irra deebi’a yaala!',
        'isalpha' => 'Sabsii (input) sirraa’e! Sabsiin (input) fudhatu qubee qofaa dha. Iddoo duwwaa dhiisuun ykn sirna tuqatti fayyadamun dhorkaa dha. Irraa deebi’a yaala!<br>',
        'isdigit' => 'Sabsii (input) dhorkame! Sabsiin (input) fudhatu lakkoofsa (figure) qofaa dha. Iddoo duwwaa dhisuun dhorka dha. Irra deebi’a yaala!<br>',
        'isalphanumeric' => 'Sabsiin (input) dhorkame! Sabsiin fudhatu kan qubee, kan kan lakkoofsaa (figure), Yookaan walmakaa lachuu dha.',
        'length' => [
            5 => 'Duwwaa dhiisun dhorkaa dha.  Irraa deebia yaala!',
            14 => 'Dheerni yoo xinnaate 14 ol tau qaba.',
        ],
        "email" => ' Sabsii dhorkame. Teessoon imeelii isin galchitan ulaagaa hin guutu. Irraa deebia yaala! ',
        "passwordRepeat" => ' Password walfakkaachu qabu. Irraa deebia yaala! ',
        "passwordNotCurrent" => ' password galchitan isa duraa wajjin walfakkaachu qaba. ',
        "invalidEmailOrPassword" => ' Imeeliin ykn password sirrii miti. Irra deebi’a yaala! ',
        "invalidEmailOrUserName" => ' Imeelichi ykn maqaan itti fayyadama sirrii miti. Irra deebi’a yaala! ',
        "mixedLanguage" => ' Sabsii dhorkame! Afaan waliinmakuun dhorka dha. Unkicha afaan filattaniin guuta. Irra deebia yaala!'
    ]
];

$message = [
    'change' =>  [
        'name' => 'Odeefanno dhuunfaa keesan Hullluutaraa wajjin walitti dhufeenya qabu unkaa arman gadi jirutti fayyadamun geeddara. Yammuu xumurtan <strong> jijjiirawwan taasiisi </strong> isa jedhu tuqa mirkanefadha. Jijjiiruf yoo barbaachuubaattan <strong> jijjiirawwan haquu </strong> Tuqun gara galmee dhuunfaa keessanitti deebi’u ni dandeessu.'
    ],
    'success' => 'Glmeen dhuunfaa keeessanii kan Huluutaraa haala milka’e jijjiirame jira!',
];

$help = [
    0 => ["HuluteraFAQ" => "Gaaffiiwwan yeroo hunda Hulluutaraa dhaan gaafataman"],
    1 => [
        "register" => [
            "head" => "Akkamitti galmaa’u akka dandeessan ",
            "body" => [
                "1) " => "Website keenya irratti kan gama mirgaatin jiru <strong> Galmaa’a</strong> Isa jedhu tuqa.",
                "2) " => "Unkaa ittiin galmaa’an irratti odeeffannnoowwan guutaa achi <button class=\"btn btn-primary\">galmaa’a</button> isaa jedhu tuqa.",
                "3) " => "Unkaa ittiin galmaa’an haala milka’e yammuu ergitan, dhaamsa kana ni argattu.<strong> Waan galmooftaniif  galatooma. Imeeli mirkanessa isiniif ergine jirra. Akaawuntii keessa hojiirra olchuuf galmee hojiira ittiin oolchan tuqa.</strong> Galmeesicha xumuruuf garaisa itti aanutti deema.",
                "4) " => "Imeelicha banaatii galmee hojiira ittiin oolchan imeeelii keenya irra tuqa Akaawuntiin keessan amma <strong>o ole jira </strong> Dhaamsa jedhu agartu. Kunis akaawuntii keessan hojjiiraoolcha kanaafuu galu ni dandeessu.",
                        "" => '<div class="help-video" style="width:100%"><p style="text-align:start">Dabalataaf vidiiyoobanaa ilaala</p>
                               <iframe width="560" height="315" src="https://www.youtube.com/embed/RU4kKKXyADo" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                               </div>',

            ]
        ],
        "logIn"    => [
            "head" => "Akkamitti galu akka dandeessan",
            "body" => [
                "1) " => "Website keenya irratti gubbaa irra gama mirgaatti kan argamu <strong> Seena </strong> isa jedhu tuqa.",
                "2) " => "Unkaa ittiin seenan irratti imeelii keessanii fi password keessan galcha. Achi booda  <button class=\"btn btn-primary\">seenaa</button> isa jedhu tuqa. Nu biraa akaawuntii yoo qabattan<strong> Akaawuntiin keessan</strong> fuula keessa ni seentu.",
            ]
        ],
        "recovery"    => [
            "head" => "Password keessan iraanfattan akkamitti argachu akka dandeessan",
            "body" => [
                "1) " => "Unkaa ittiin seenan gaditti<strong> Password keessanii ni iraanfattanii?</strong> isa jedhu tuqa. Fuulli password ittiin yaadatan yammuu banamu maqaa ittiin fayyadamtan ykn imeelii keessa erga isa jedhu tuqa. Achis itti fufuun dhaamsa kana ni argitu<strong>Odeefannoo password ittiin jijjiiratan imeelii dhana isiniif ergine jirra.</strong>.qajeelfama imeelicha irra jiru sirriitti hordoofa. Adeemsa password jijjiiruu haal milk’aina qabun xumuruuf gara isa itti aanutii deema.",
                "2) " => "Imeelii keessan banatii imeelii keenya isiniif ergame irratti password kan ittiin jijjiiran galmee tuqa password keessan kana isin ni beeksisa. Kana <strong>password fuulaakaawuntii keessan keessa seenatanii jijjiiru ni dandeessu</strong>።password keessan isa haara galchun haala milkaa’ina qabun ni xumurtu. Kanaafuu password isaa haaraa dhaan seenu ni dandeessu. ",
                        "" => '<div class="help-video" style="width:100%"><p style="text-align:start"><a onclick="playVideo(\'recovery\')" href="javascript:void(0)">Dabalataaf vidiiyoo ilaala</a></p>
                                <video width="50%" id="vid-recovery" style="border: 2px black solid">
                                <source src="../videos/am/pass_rec_am.mp4" type="video/mp4">
                                </video>
                              </div>',
            ]
        ],
        "upload"   => [
            "head" => "Akkamiti qabeenya galchuu akka dandeessaan",
            "body" => [
                "1) " => "Qabeenya galchuuf jalqaba galu qabdu.",
                "2) " => "Eega galtan booda gubbarra gama mirgaatiin kan jiru<strong>qabeenya galcha </strong>isa jedhu tuqa. Achii itti aansun qabeenya galchu barbaaddan filadhaa. ",
                "3) " => 'Achi itti aansuun qabeenya galchu barbaaddan filadhaa. Unkaan qabeenya irratti galchan ni banama. Unkicha irratti waa’ee qabeenyaa odeefannoo hunda galchaa. Hanga isiniif danda’ametti suuraa bareeda galchuuf yoo dandeessan ija nama heedduu harkisuuf isn gargaara. Kana bira darbee haalaqabeenyichi irratti argamu ifatti agarsiisuuf ni gargaara. Hanga suura shan galchu ni dandeesssu. Egaa xumurtan booda <button class="btn btn-primary">erga, galchaa, tuqa </button> isa jedhu jedhu tuquun haalamilkaa’ina qabun xumura. Itti aansees<strong> qabeenyawwan keessan bifa milkaa’ina qabun gale jira. Tajaajila keenyatti waan itti fayyadamtaniif ni galatefanna.</strong>dhaamsa jedhu argitu.  Bu’uura kanaanis qabeenya isin galchitan qabeenya kiyya kan jedhu keessatti argattu. Qabeenyichi sa’aatii 24 keessatti hojiirra oolee nama hundaf kan mul’atu ta’a.',
                         "" => '<div class="help-video" style="width:100%"><p style="text-align:start">Dabalatan vidiiyoo ilaala</p>
                                <iframe width="560" height="315" src="https://www.youtube.com/embed/Rn0cHQny4nU" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                                </div>',

            ]
        ],
        "contactus"   => [
            "head" => "Akkamitti nu gaafachu akka dandeessan",
            "body" => [
                "1) " => "Website keenya gubbaa gama mirgaatiin ykn jala kan jiru<strong>gaafachu ni barbaadduu?</strong>isa jedhu tuqa.",
                "2) " => "<strong>Dhaan sababii ittiin nu haasofsiisu barbaaddan</strong> tarreeffama irra sababii keessan yoo argachubattan, <strong>yaada waliigalaa</strong>isa jedhu filadhaa.Seentanii yoo ta’e odeefannon keessan matuma isatiin ni guuta. Yoo ta’ubaate odeefannicha gutaa.",
                "3) " => "Unkaa eega guuttan booda  <button class=\"btn btn-primary\">erga </button> isa jedhu tuqa. Itti aansuun<strong> argachu waan barbaaddani  galtooma</strong> dhaamsa jedhu argattu.",
                        "" => '<div class="help-video" style="width:100%"><p style="text-align:start"><a onclick="playVideo(\'contactus\')" href="javascript:void(0)">Dabalataan vidiyoo</a></p>
                                <iframe width="560" height="315" src="https://www.youtube.com/embed/oXuDJPOqdSg" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                               </div>',
            ]
        ],
        "search"   => [
            "head" => "Qabeenya akkamitti barbaachuu akka dandeessan",
            "body" => [
                "" => "Website keenya irra qabeenya barbaaddan haala salphaan argachu ni dandeessu. Kanas unkaa ittiin barbaadan irratti odeefannoowwan galchuun milkaa’un ni dandeessu. Application keenya  ittiin hojiirra oolcha bu’uuraodeefannoo galchitaniin bu’a adda addaa ni kenna. Kunis akka armana gadi ta’a<br /> 1) <strong>gujecha ittiin barbaadan</strong>yoo galubaate<strong>qabeenyawwan hunda</strong>fi<strong>magaaloota hunda</strong>yoo filatame. Iti aansuun <button class='btn btn-warning'>Haa barbaadamu</button>isa jedhu tuqa.Qaneenyawwan hunda Website keenya irra jirankanneen hojiirra oolan ni agarsiisa.<br /> 2) <strong>Jecha ittiin barbaadan</strong>yoo gale, <strong>qabeenyawwan hunda </strong>fi<strong>magaaloota hunda</strong>yoo filatame. Itti aansuun <button class=\"btn btn-warning\">haa barbaadamu</button> isa jedhu tuqa. Qbeenyawwan website keenya irra jiran odeefannoo isin galchitan kanneen guutan ni agarsiisa. Kunis magaaloota hundaa fi akaakuu qabeenyawwan hunda ni hammata. <br /> 3) <strong> jecha ittiin barbaadan</strong>Yoo galubaate,<strong>konkolaataa/kompiyuutara/eelektoniksii/mana/meeshaalee manaa/bilbila/kanneen biroo</strong>qabeenya dabalatan filattan ilaalchisee odeefannon dabalataan ittiin barbaadan ilaalchisee yoo filatamee fi<strong>magaaloota hunda</strong>hun yoo filatame. Itti aansuun isa <button class=\"btn btn-warning\">haa barbaadamu</button> jedhu tuqa. Qabeenyawwan website keenya irra jiran odefannoo isin galchitan kaneen guutan ni agarsiisa. Kunis qabeenya barbaaddan hunda magaaloota hunda keessa barbaachu ni hammata.<br /> 4) <strong>jecha ittiin barbaadan</strong> Yoo gale <strong> konkolaataa/kompiyuutara/eelektoniksii/mana/meeshaaleemanaa/bilbila/kanneen biroo</strong> Kana malees qabeenya filattan ilaalchisee odeefannoon ittiin barbaadamu yoo filatameefi<strong>magaalaan tokko yoo filatame</strong>yoo filatame, itti aansun <button class=\"btn btn-warning\">haa barbaadamu</button> isa jedhu tuqa. Qabeenyawwan website keenya irra jiran keessee kanneen  odeefannoo isin galchitan kan guutan ni agarsiisa. Kunis qabeenya isin filattan magalaaf latame keessaa filachu ni hammata.<br />",
                "i) " => "Saanduqa ittiin barbaadan website keenya irra gubbaa gama mirgaatiin argama.",
                "ii) " => "Odeefannoowwan ittiin barbaadan hunda galcha.",
                "iii) " => "Itti aansuun haa barbaadamu isa jedhu tuqa",
                "*" => "Yammuu jecha ittiin barbaadamu galchitan jecha gabaabaatti yoo fayyadamtan ni filatama. Qabeeenya achi jiru argachuuf carraan bal’dhaa dha. Fakeenyaaf konkoolaataa diimaa barbaaduuf, diimaa jettanii yoo barreesitan  filatama dha.Afaan adda addaatiin barbaadunis qabeenya dabalata agarsiisu ni danda’a. Afaan adda addaaatiin barbaachuunis qabeenyawwan dabalata agarsiisu ni danda’a.",
                "**" => "Qabeenyi barbaadame yoo argamubaate<strong> qabeenyichi hin argamne</strong> dhaamsa kan jedhu ni mul’ata.",
            ]
        ]
    ]
];



$GLOBALS['lang'] = $lang;
$GLOBALS['city_lang_arr'] = $city_lang_arr;
$GLOABLS['addis_sub_city'] = $addis_sub_city;
$GLOABLS['item_lang_arr'] = $item_lang_arr;
$GLOABLS['upload_specific_array'] = $upload_specific_array;
$GLOABLS['abuse_type_lang_arr'] = $abuse_type_lang_arr;
$GLOABLS['user_specific_array'] = $user_specific_array;
$GLOABLS['validate_specific_array'] = $validate_specific_array;
$GLOABLS['message'] = $message;
$GLOABLS['help'] = $help;





?>