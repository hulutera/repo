...
if(!empty($passRecovery) && !empty($key)){
	...
	if (mysqli_num_rows($result)==0) {
        ...
        $endDate = 2592000 ;
		$table = "tempuser";
		$condition = "WHERE 'tuID' = " . $user_id;
        if ($today - $regDateSrting >= $endDate ){
			$resulst2 = DatabaseClass::getInstance()->deleteUser($table, $condition);
			header ('Location: ../includes/prompt.php?type=11');
        }
		else{
            $result1 = DatabaseClass::getInstance()-> insertUserInfo($table, $username, $email, $password, $firstname, $lastname, $phone, $address, "");
			if(!$result1){
