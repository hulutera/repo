...
SQL;
    ...
    */
    ...
    private function inputFieldCamera()
    {
        $errorDiv = "";
        if(isset($_SESSION['errorRaw']['fieldCamera']))
        {
            $errorDiv = '<div style="color:red;">'. $_SESSION['errorRaw']['fieldCamera'] .'</div>';
        }
        echo '
        <div class="col-md-4">
        <div class="form-group">
        '.$errorDiv.'
           <label for="fieldCamera">Camera Size</label> 
           <div>
             <select id="fieldCamera" name="fieldCamera" class="select form-control">';
        $choose = "Choose Camera Size";
        if (isset($_SESSION['POST']['fieldCamera'])) {
            $choose = $_SESSION['POST']['fieldCamera'];
        }
        ...
        $make = array(
            ...
            "more than 16 megapixles" => "more than 16 megapixles");
        foreach ($make as $key => $value) {
            echo '<option value="' . $key . '">' . $value . '</option>';
        }
        echo '</select></div></div></div>';
